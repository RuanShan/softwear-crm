...
describe QuoteHelper do
  describe '#quantities_and_decoration_prices', story_729: true do
    context "given a quote's imprintable jobs" do
      let!(:job_1) { create(:quote_job, line_items: [create(:imprintable_line_item, decoration_price: 20, quantity: 3)]) }
      let!(:job_2) { create(:quote_job, line_items: [create(:imprintable_line_item, decoration_price: 10, quantity: 5)]) }
