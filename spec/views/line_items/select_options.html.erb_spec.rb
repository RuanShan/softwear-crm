...
describe 'line_items/select_options.html.erb', line_item_spec: true do
	[Brand, Imprintable, Color].each do |type|
		context "with #{type.name.pluralize}" do
			5.times do |n|
				let!("object#{n}") { create "valid_#{type.name.underscore}" }
			end

			it "renders a select field for the #{type.name.pluralize}" do
		  	render template: 'line_items/select_options',
		  	       locals: { objects: type.all, type_name: type.name },
		  	       layout: nil

		  	expect(rendered).to have_css "select[name='#{type.name.underscore}_id']"
		  	expect(rendered).to have_css "label[for='#{type.name.underscore}_id']",
		  	                             text: type.name
	...
	context 'when there are no objects' do
		it 'apologizes' do
			render template: 'line_items/select_options',
			       locals: { objects: Brand.all, type_name: Brand.name },
			       layout: nil
