module InsightlyHelper
  def insightly_categories
    ...
    client.get_task_categories.map { |t| [t.category_name, t.category_id] }
  end

  def insightly_pipelines
    return nil unless insightly_available?

    client.get_pipelines.map { |t| [t.pipeline_name, t.pipeline_id] }
  end

  def insightly_opportunity_profiles
    return nil unless insightly_available?

    client.get_custom_field(id: 'OPPORTUNITY_FIELD_12')
  ...
  def insightly_bid_tiers
    ...
    client.get_custom_field(id: 'OPPORTUNITY_FIELD_11')
      .custom_field_options
      .map { |f| [f['option_value'], f['option_id']] }
  end

  def insightly_select(f, field, options)
    f.select field, options_for_select(options || []), {}, { disabled: !insightly_available? }
  end
  ...
  def client
    if current_user.insightly_api_key
      @client ||= Insightly2::Client.new(current_user.insightly_api_key)
    end
  end
end
