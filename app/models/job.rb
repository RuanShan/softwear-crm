  ...
  def self.assign_line_items_attributes_proc(tier_num = nil)
    proc do |attrs|
      ...
      attrs.each do |key, line_item_attributes|
        ...
        if /\d+/ =~ key
          # Although, if there is only an ID, we do not want to bother (because there are
          # hidden ID fields generated by rails' form_for)
          next if line_item_attributes.keys == ['id']
