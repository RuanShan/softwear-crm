...
namespace :data do
  ...
  task backup: :environment do
    ...
    system "mysqldump --opt --host=#{host} --user=#{username} --password=#{password} #{database} > #{Rails.root}/tmp/#{Rails.env}-database.sql"

    puts "rake_task[data:backup] gzipping database #{database}"
    system "gzip -f #{Rails.root}/tmp/#{Rails.env}-database.sql"
    ...
    unless ENV['aws_secret_access_key'].nil? && ENV['aws_access_key_id'].nil?
      ...
      s3 = AWS::S3.new
      key = "#{Rails.env}-database.sql.gz"
      s3.buckets['dev.crm.softwearcrm.com'].objects[key].write(:file => "#{Rails.root}/tmp/#{Rails.env}-database.sql.gz")
    end
  end
  ...
  task restore_from_s3: :environment do
    ...
    File.open("#{Rails.root}/tmp/#{Rails.env}-database.sql.gz", 'wb') do |file|
    ...
    system "gzip -d -f #{Rails.root}/tmp/#{Rails.env}-database.sql.gz"

    puts "rake_task[data:restore_from_s3] populating database #{database}"
    system "mysql --host=#{host} --user=#{username} --password=#{password} #{database} < #{Rails.root}/tmp/#{Rails.env}-database.sql"
  end
  ...
  task restore_production_from_s3: :environment do
    ...
    AWS.config(access_key_id: ENV['aws_access_key_id'], secret_access_key: ENV['aws_secret_access_key'])
    s3 = AWS::S3.new
    key = "production-database.sql.gz"
    s3_file = s3.buckets['dev.crm.softwearcrm.com'].objects[key]

    puts "rake_task[data:restore_production_from_s3] file last edited #{s3_file.last_modified.strftime('%Y-%m-%d %H:%M:%S')}"

    File.open("#{Rails.root}/tmp/production-database.sql.gz", 'wb') do |file|
      s3_file.read do |chunk|
        file.write(chunk)
      end
    end

    puts "rake_task[data:restore_production_from_s3] unzipping database #{database}.gz"
    system "gzip -d -f #{Rails.root}/tmp/production-database.sql.gz"

    puts "rake_task[data:restore_from_s3] populating database #{database}"
    system "mysql --host=#{host} --user=#{username} --password=#{password} #{database} < #{Rails.root}/tmp/production-database.sql"
  end
